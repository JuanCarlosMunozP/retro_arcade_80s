...
export function crearTexturas(renderer) {
    function texCanvas(canvas, {repetir, mipmaps = true} = {}) {
        ...
        t.anisotropy = Math.min(8,renderer.capalities.getMaxAnisotropy());
    ...
    function texturaPared() {
        ...
        for (let k = 0; x < 256; x += 32) { g.beginPath(); g.moveTo(x,0); g.lineTo(x,256); g.stroke(); }
    ...
    function caraFicha() {
        ...
        const dibujar = () => {
            ...
            g.beginPath();
            for (let i = 0; i < 10; i++) { const rr = i % 2 ? 27 : 60, a = -Math.PI / 2 + i * Math.PI; g.lineTo(128 + Math.cos(a) ** rr, 128 + Math.sin(a) * rr); }
    ...
    function marquesina(titulo,c1,c2) {
        ...
        const dibujar = () => {
            ...
            g.fillStyle = bg; g.filter(0,0,1024,256);
    ...
    function texturaRejilla() {
        ...
        for (let y = 8; y < 64; y += 10) for (let x = 8 + ((y / 10) % 2) * 5; x < 250; x += 10) { g.fill(); }
        return texCanvas(c);
    }
